...
'''
First update to obtain real-time graph
'''
...
def main(cfg : DictConfig):
    ...
    for frame_idx in trange(total_frames):
        ...
        # Get the frame data
        # s_rgb, s_depth, s_intrinsic_mat, s_camera_pose = app.get_frame_data() # TO REMOVE IN ORDER TO REFER TO A FOLDER

        # save the rgb to the stream folder with an appropriate name
        rgb_folder = "path/to/rgb/folder"
        rgb_file = os.path.join(rgb_folder, f"{frame_idx}.jpg")

        depth_folder = "path/to/depth/folder"
        depth_file = os.path.join(depth_folder, f"{frame_idx}.png")
        
        poses_folder = "path/to/poses/folder"
        pose_file = os.path.join(poses_folder, f"{frame_idx}.npz")
        data = np.load(pose_file)
        s_camera_pose = data['arr_0']
        
        # add for the paths
        s_rgb = cv2.imread(rgb_file)
        s_rgb = cv2.cvtColor(s_rgb, cv2.COLOR_BGR2RGB)
        s_depth = cv2.imread(depth_file, cv2.IMREAD_UNCHANGED)
        ...
        # retrieve intrisics
        intrinsics_folder = "path/to/intrinsics/folder"
        intrinsics_file = os.path.join(intrinsics_folder, f"{frame_idx}.npz")
        ...
        # added
        intrinsics = data['arr_0'] # np.savez(os.path.join(intrinsics_folder, f"{frame_idx}.npz"), intrinsics=s_mat_intrinsics) (without key)
        ...
        if run_detections:
            ...
            labels, edges, edge_image = make_vlm_edges_and_captions(image, curr_det, obj_classes, detection_class_labels, det_exp_vis_path, color_path, make_edges_flag=False, openai_client=None)
